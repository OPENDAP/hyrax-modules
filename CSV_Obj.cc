...
bool
CSV_Obj::open( const string& filepath )
{
...
CSV_Obj::load()
{
	...
	else if( !txtLine.empty() )
	{
		...
		{
		    ostringstream err ;
		    err << " Attempting to add value " << token
		        << " to field " << index << ", field does not exist" ;
...
void
CSV_Obj::getFieldList( vector<string> &list )
{
    _header->getFieldList( list ) ;
}

string
CSV_Obj::getFieldType(const string& fieldName)
{
    return _header->getFieldType( fieldName ) ;
}

int
CSV_Obj::getRecordCount()
{
...
{ 
	...
	{
	    string err = (string)"Unable to get data for field "
			 + field ;
...
vector<string>
CSV_Obj::getRecord( const int rowNum )
{
    ...
    {
	ostringstream err ;
	err << "Attempting to retrieve row " << rowNum << " of "
	    << maxRows ;
	throw BESInternalError( err.str(), __FILE__, __LINE__ ) ;
    }
	...
	{
	    ostringstream err ;
	    err << "Unable to retrieve data for field " << fieldName
	        << " on row " << rowNum ;
	    throw BESInternalError( err.str(), __FILE__, __LINE__ ) ;
	}
	type = f->getType();
...
void
CSV_Obj::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "CSV_Obj::dump - ("
	 << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
    if( _reader )
    {
