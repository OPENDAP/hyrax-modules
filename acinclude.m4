...
AC_DEFUN(DODS_GET_DODS_ROOT, [dnl
	...
	[The full path to the root of the DODS source tree])
    AC_SUBST(dods_root)
    AC_DEFINE_UNQUOTED(DODS_RELATIVE, "$dods_relative",
	[The relative path to the root of the DODS source tree])
...
# supports decompression. So we just use our own copy. This will change in 
# about a year. 09/20/02 jhrg
AC_DEFUN(DODS_WWW_LIB, [dnl
    AC_REQUIRE([DODS_GET_DODS_ROOT])
    AC_REQUIRE([DODS_PACKAGES_SUPPORT])
    AC_MSG_CHECKING([for libcurl and its headers])
...
# These macros used to test for various versions; we don't use them anymore.
# 09/20/02 jhrg
...
# Check for support of `-gstabs' by gcc (SGI does not support it unless you're 
...
AC_DEFUN(DODS_GCC, [dnl
    ...
    GCC_ROOT=`gcc --version 2>&1 | awk '/specs/ {print}'`
